  ...
  useEffect(() => {
    ...
    class WispEntity {
      ...
      draw() {
        ...
        ctx.scale(1, 0.22)
        ctx.beginPath()
        ctx.arc(this.x, this.y / 0.22, this.rx, 0, Math.PI * 2)
        ctx.fillStyle = g
        ctx.fill()
        ctx.restore()
      }
    ...
    // ── Init entities ──
    const ghosts: GhostEntity[] = Array.from({ length: 14 }, () => new GhostEntity())
    const wisps: WispEntity[] = Array.from({ length: 20 }, () => new WispEntity())
    ...
    function bgLoop() {
      ...
      pts.forEach((a, i) => pts.slice(i + 1).forEach(b => {
        ...
        if (d < 95) {
          ctx.strokeStyle = `rgba(91,33,182,${0.09 * (1 - d / 95)})`
          ctx.lineWidth = 0.5
